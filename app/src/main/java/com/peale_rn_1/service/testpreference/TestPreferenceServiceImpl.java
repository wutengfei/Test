...
import com.peale_rn_1.dao.usertest.UserTestDao;
...
public class TestPreferenceServiceImpl implements TestPreferenceService {
    ...
    TestPreferenceDaoImpl testPreferenceDao = new TestPreferenceDaoImpl(MainApplication.getContext());
    UserTestDao userTestDao = new UserTestDaoImpl(MainApplication.getContext());
        ...
        if (testlist != null && testlist.size() > 3) {
            ...
            for (i = 0; i < 4; i++) {
                for (j = 0; j < 3; j++) {
                    ...
                    if (aspectcount[i][j] == 0) {
                        aspectcount[i][j]++;
                        count++;
                    }
                    if (i < 3 && difficultycount[i][j] == 0) {
                        difficultycount[i][j]++;
                        count++;
                    }
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (i < 3) {
                ...
                if (tp3 != null) {
                    ...
                    testPreferenceDao.save(new TestPreference(userId, "testDifficulty", t, Fdifficulty[i][0], Fdifficulty[i][1],
                            Fdifficulty[i][2]));
            }
        }
    ...
    public double recommendByPreference(String userId, int testType, int testAspect, int testDifficulty) {
        ...
        if (tp != null && type != null && aspect != null && difficulty != null) {
            return (tp.getpFeedback1() * type.getpFeedback1() * aspect.getpFeedback1() * difficulty.getpFeedback1()) * 2 +
                    (tp.getpFeedback2() * type.getpFeedback2() * aspect.getpFeedback2() * difficulty.getpFeedback2()) -
                    (tp.getpFeedback3() * type.getpFeedback3() * aspect.getpFeedback3() * difficulty.getpFeedback3());
        }
        return 0;
    }
